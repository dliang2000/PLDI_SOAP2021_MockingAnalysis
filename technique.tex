...
\paragraph{Interprocedural support} Heros\cite{bodden12:_inter_proced_data_flow_analy} implements IFDS/IDE for the Soot program analysis framework. We have explored implementing our analysis in Heros, but this would be a more involved process than in the declarative case, where we simply added two rules to obtain the interprocedural analysis. We decided that, for our purposes, the extra effort required to formulate our analysis as IFDS would not give us any additional understanding of the problem; comparing the Doop intraprocedural and interprocedural results was sufficient.
...
\end{itemize}
False positives (threatening precision) result from the approximations above. In addition to what is mentioned above, we are also vulnerable to false negatives (threatening soundness) from missed collection APIs and mock APIs. 
